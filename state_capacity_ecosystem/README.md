...
4. **Mamdani AI priorities post URL is a placeholder** (`substack/mamdani-ai-priorities/`). The prototype set is FINAL as of 2026-07-29 (3 live: grocery siting, summons navigator, housing approvals navigator; the other 6 sketched slots were cut). Remaining when the post publishes: swap the hero's "Post coming ↗" `.pending` `href="#"` row for the SCE Substack post URL, and change the hub card's "Forthcoming post · Prototypes" kicker to the post month.

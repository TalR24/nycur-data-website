...
4. **Mamdani AI priorities post page has placeholders** (`substack/mamdani-ai-priorities/`, added 2026-07-29 ahead of the post). (a) The hero's "Post coming ↗" row is `.pending` `href="#"`; swap in the SCE Substack post URL when published. (b) Eight prototype slots are dashed `.pending` cards awaiting collaborator uploads to the work repo's `substack_projects/` folder; as each arrives, nest it at `substack/mamdani-ai-priorities/<tool-slug>/` and convert its slot to a live linked card. (c) The hub card says "Forthcoming post · Prototypes" and "1 live · 8 in the works"; update both as prototypes ship and the post publishes.
